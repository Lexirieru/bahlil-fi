...
public struct MockedSUI has key { id: UID }
...
public fun mint(
    treasury_cap: &mut TreasuryCap<MockedSUI>,
    amount: u64,
    recipient: address,
    ctx: &mut TxContext,
) {
...
public fun burn(currency: &mut Currency<MockedSUI>, coin: Coin<MockedSUI>, ctx: &TxContext) {
    // Only the admin address can burn tokens
    assert!(tx_context::sender(ctx) == ADMIN_ADDRESS, E_NOT_AUTHORIZED);
